...
function getOllamaModelConfig(): {ollamaModel: Model<'openai-completions'>, options: ProviderStreamOptions} {
    // 配置模型基本信息
...
async function showModelUsage(): Promise<void> {
    ...
    const context: Context = {
        ...
        messages: [
            {role: "user", content: "你好啊", timestamp: Date.now()},
        ]
    }
...
async function showToolCalling() {
    ...
    const tool: Tool = {
        ...
        parameters: Type.Object({
            timezone: Type.Optional(Type.String({ description: "Timezone identifier, e.g. 'UTC', 'America/New_York', default 'Asia/Shanghai'" }))
        })
    };
    ...
    const context: Context = {
        ...
        messages: [
            { role: "user", content: "你好，请问现在是什么时间", timestamp: Date.now() }
        ],
        tools: [tool]
    };
